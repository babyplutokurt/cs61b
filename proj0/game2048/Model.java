...
public class Model extends Observable {
    ...
    public static boolean emptySpaceExists(Board b) {
        ...
        int check_size = b.size();
        for (int row = 0; row < check_size; row += 1) {
            for (int col = 0; col < check_size; col += 1) {
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int row = 0; row < check_size; row += 1) {
            for (int col = 0; col < check_size; col += 1) {
                int current_value = b.tile(row, col).value();
                for (int move = 0; move < 4; move++) {
                    int new_row = row + dx[move];
                    int new_col = col + dy[move];
                    if (new_col > 0 && new_col < check_size && new_row > 0 && new_row < check_size) {
                        if (current_value == b.tile(new_row, new_col).value()) {
                            return true;
                        }
                    }
                }
